...
def segment_mse_helper(expected, prediction, signal_ranges, non_sig_ranges):
    w1 = 1
    w2 = 0.7
    squares = tf.math.square(expected - prediction)
    squares = squares.numpy()
    for sig_range in signal_ranges:
        squares[sig_range[0]:sig_range[1]] = w1*squares[sig_range[0]:sig_range[1]]
    for non_sig_range in non_sig_ranges:
        squares[non_sig_range[0]:non_sig_range[1]] = w2*squares[non_sig_range[0]:non_sig_range[1]]
    
    mse = tf.math.reduce_mean(squares)
